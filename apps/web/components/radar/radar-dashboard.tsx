...
import { ArrowDown, ArrowUp, Bookmark, ChevronLeft, ChevronRight, Search, X } from "lucide-react";
...
export function RadarDashboard(){
  ...
  return <div className="space-y-5">
    <PageHeader title="AI Business Radar" description="Emerging AI business opportunities ranked by evidence, momentum and commercial signals." actions={fetchedAt?<span className="hidden text-xs text-slate-500 sm:block">本页查询于 {fetchedAt.toLocaleTimeString("zh-CN",{hour:"2-digit",minute:"2-digit"})}</span>:undefined}/>
    <section aria-label="当前结果摘要" className="grid grid-cols-2 gap-3 lg:grid-cols-4"><StatCard label="当前页机会" value={data?String(data.items.length):"—"}/><StatCard label="当前页平均 Score" value={data?average(scores):"—"}/><StatCard label="当前页平均 Confidence" value={data?average(confidences):"—"}/><StatCard label="符合条件总数" value={data?String(data.total):"—"}/></section>
    ...
    <form key={serialized} onSubmit={submitFilters} className="rounded-xl border bg-white p-4 shadow-sm dark:bg-slate-950"><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      ...
      <TextFilter name="industry" label="Industry" value={params.get("industry")}/><TextFilter name="business_model" label="Business Model" value={params.get("business_model")}/><TextFilter name="customer_type" label="Customer Type" value={params.get("customer_type")}/>
      <label><span className="mb-1 block text-xs font-medium text-slate-500">Market Stage</span><select name="market_stage" defaultValue={params.get("market_stage")??""} className="h-10 w-full rounded-md border bg-transparent px-3 text-sm"><option value="">全部阶段</option>{STAGES.map(stage=><option key={stage}>{stage}</option>)}</select></label>
      <NumberFilter name="score_min" label="Score minimum" value={params.get("score_min")}/><NumberFilter name="confidence_min" label="Confidence minimum" value={params.get("confidence_min")}/><NumberFilter name="hype_max" label="Hype maximum" value={params.get("hype_max")}/>
      <div className="flex items-end gap-2 xl:col-span-3"><button className="h-10 rounded-md bg-slate-950 px-5 text-sm font-medium text-white dark:bg-white dark:text-slate-950">应用筛选</button>{hasFilters&&<button type="button" onClick={clearFilters} className="flex h-10 items-center gap-1 rounded-md border px-4 text-sm"><X size={14}/>清除筛选</button>}</div>
    </div></form>
    ...
    {loading?<DataTableShell><SkeletonTable/></DataTableShell>:error?<ErrorState message={error.status===403?"你没有查看 Radar 的权限":error.message} requestId={error.requestId??undefined} onRetry={()=>setRefreshKey(value=>value+1)}/>:data&&data.items.length===0?<EmptyState title={hasFilters?"没有符合筛选条件的机会":"目前还没有已评分的机会"} description={hasFilters?"调整或清除筛选条件后重试。":"完成信号提取、聚合与评分后，机会会显示在这里。"} action={hasFilters?<button onClick={clearFilters} className="mt-4 rounded-md border px-4 py-2 text-sm">清除筛选</button>:undefined}/>:data&&<RadarResults items={data.items} sort={sort} direction={direction} onSort={changeSort}/>} 
...
function TextFilter({name,label,value}:{name:string;label:string;value:string|null}){return <label><span className="mb-1 block text-xs font-medium text-slate-500">{label}</span><input name={name} defaultValue={value??""} className="h-10 w-full rounded-md border bg-transparent px-3 text-sm"/></label>}
function NumberFilter({name,label,value}:{name:string;label:string;value:string|null}){return <label><span className="mb-1 block text-xs font-medium text-slate-500">{label}</span><input name={name} type="number" min="0" max="100" step="1" defaultValue={value??""} className="h-10 w-full rounded-md border bg-transparent px-3 text-sm tabular-nums"/></label>}
