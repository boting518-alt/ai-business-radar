...
export function OpportunityDossier({identifier}:{identifier:string}){
  ...
  const evidenceType=params.get("evidence_signal_type")||"";
  const [detail,setDetail]=useState<OpportunityDetail|null>(null);
  const [trends,setTrends]=useState<TrendItem[]>([]);
  const [scores,setScores]=useState<ScoreItem[]>([]);
  const [evidence,setEvidence]=useState<EvidencePage>({items:[],total:0,offset:0,limit:EVIDENCE_LIMIT,has_more:false});
  ...
  },[evidenceType,evidenceOffset,identifier,locale,retry,router,trendWindow]);
  ...
  return <article className="space-y-7">
    ...
    <Section title={t("evidence.title")} actions={<Link className="text-sm text-cyan-700 hover:underline" href={`/signals?opportunity_id=${opportunity.id}`}>{t("evidence.related")}</Link>}>
      <p className="text-xs text-slate-500">{t("evidence.countHint")}</p>
      <label className="flex items-center gap-3 text-sm">{t("signals.type")}<select aria-label={t("signals.type")} value={evidenceType} onChange={event=>updateUrl({evidence_signal_type:event.target.value,evidence_offset:null})} className="rounded-md border bg-transparent p-2"><option value="">{t("signals.allTypes")}</option>{["pain","purchase_intent","pricing","revenue","competition","workflow"].map(type=><option key={type} value={type}>{enumLabel(type)}</option>)}</select></label>
      <p className="text-sm text-slate-500">{t("evidence.total")}: {evidence.total}</p>
      <EvidenceList items={evidence.items} emptyMessage={evidenceType||evidenceOffset?t("evidence.filteredEmpty"):detail.evidence_summary.active_signal_count>0?t("evidence.incomplete"):t("evidence.empty")}/>
      <div className="mt-3 flex justify-end gap-2"><button type="button" disabled={evidenceOffset===0} onClick={()=>updateUrl({evidence_offset:String(Math.max(0,evidenceOffset-EVIDENCE_LIMIT))})} className="flex items-center gap-1 rounded-md border px-3 py-2 text-sm disabled:opacity-40"><ChevronLeft size={15}/>{t("signals.previous")}</button><button type="button" disabled={!evidence.has_more} onClick={()=>updateUrl({evidence_offset:String(evidenceOffset+EVIDENCE_LIMIT)})} className="flex items-center gap-1 rounded-md border px-3 py-2 text-sm disabled:opacity-40">{t("signals.next")}<ChevronRight size={15}/></button></div>
    </Section>
    ...
    <Section title="Score history">{scores.length>1?<HistoryTable headings={["Calculated","Opportunity Score","Confidence","Hype Risk"]} rows={scores.map(score=>[date(score.calculated_at),display(score.opportunity_score),display(score.confidence_score),display(score.hype_risk_score)])}/>:<EmptyState title={scores.length?"Current score available; historical scoring has not accumulated yet.":"Score history is not available yet."} description={scores.length?`当前评分日期：${date(scores[0].calculated_at)}`:"该机会还没有评分历史。"}/>}</Section>
