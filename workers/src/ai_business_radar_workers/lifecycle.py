...
@asynccontextmanager
async def opportunity_normalization_dependencies(settings: WorkerSettings):
    if settings.ai_provider != "openai" or not settings.ai_model_opportunity_normalization:
        raise AIConfigurationError(
            "AI opportunity normalization provider and model are not configured"
        )
    if settings.openai_api_key is None:
        raise AIConfigurationError("OPENAI_API_KEY is not configured")
    engine = create_database_engine(settings.database_url.get_secret_value())
