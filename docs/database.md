...
**Important constraints:** `review_type IN ('signal_validation', 'opportunity_match', 'opportunity_merge', 'opportunity_creation', 'opportunity_activation', 'hype_review', 'quality_review')`; `target_type IN ('signal', 'opportunity')`; `status IN ('pending', 'in_review', 'resolved', 'ignored')`; decision null or one of `approve`, `merge`, `create_new`, `reject`, `ignore`, `defer`; `priority >= 0`; resolved/ignored tasks require `resolved_at` and a decision; open tasks have no resolved timestamp. Migration `0013_opportunity_activation.sql` extends only the review-type constraint.
