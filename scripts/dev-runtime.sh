...
(cd "$PROJECT_ROOT" && uv run --project workers uvicorn ai_business_radar_api.main:app --reload) &
pids="$pids $!"
(cd "$PROJECT_ROOT" && uv run --project workers dramatiq ai_business_radar_workers.worker) &
pids="$pids $!"
(cd "$PROJECT_ROOT" && uv run --project workers python -m ai_business_radar_workers.scheduler) &
pids="$pids $!"
